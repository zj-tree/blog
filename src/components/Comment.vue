      ...
      >
        ...
        <div class="comment-meta">
          ...
          <div class="comment-info">
            ...
            <!-- 点赞 -->
            <span
              :class="isLike(item.isLike) + ' iconfont icondianzan'"
              @click="like(item)"
            />
          ...
          >
            ...
            <div class="reply-meta">
              ...
              <div class="comment-info">
                ...
                <!-- 点赞 -->
                <span
                  :class="isLike(reply.isLike) + ' iconfont icondianzan'"
                  @click="like(reply)"
                />
          ...
          >
            <span style="padding-right:10px">
              共{{ Math.ceil(item.replyDto.length / 5) }}页
            </span>
            <paging
              ref="page"
              :totalPage="Math.ceil(item.replyDto.length / 5)"
              :index="index"
...
import { getComment, addComment, reComment } from "@/api/comment";
import { likeCommon } from "@/api/common";
...
export default {
  ...
  methods: {
    ...
    async checkReplies(index, item) {
      console.log("item" + item);
      console.log("id:" + item.commentId);
      console.log(this.$refs.check[index].style.display);
      // await addComment()
      // this.axios
    ...
    changeReplyCurrent(current, index, commentId) {
      ...
      //   })
      //   .then(({ data }) => {
      //     this.commentList[index].replyDto = data.data;
      //   });
    },
    ...
    async like(comment) {
      ...
      const { data } = await likeCommon(comment.commentId);
      if (data.flag) {
    ...
    async reloadReply(index) {
      ...
      this.commentList[index].replyDto = data;
      // this.axios
  ...
  computed: {
    isLike() {
      return function(isLikeFlag) {
        // var commentLikeSet = this.$store.state.commentLikeSet;
        return isLikeFlag === "1" ? "like-active" : "like";
      };
    }
  },
  watch: {
