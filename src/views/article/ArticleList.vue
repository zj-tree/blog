          ...
          <v-card class="animated zoomIn article-item-card">
            ...
            <div class="article-item-info">
              ...
              <div style="margin-top:0.375rem">
                ...
                <!-- 文章分类 -->
                <router-link :to="'/categories/' + typeId" class="float-right">
                  <v-icon>mdi-bookmark</v-icon>{{ item.type }}
                </router-link>
              </div>
            </div>
            <!-- 分割线 -->
...
<script>
import { listByTypeId, listByFlag } from "@/api/article";
export default {
  created() {
    ...
    this.typeId = this.$route.params.categoryId;
  ...
  methods: {
    async infiniteHandler($state) {
      ...
      const { data } = params.categoryId
        ? await listByTypeId(params.categoryId, { pageNum: this.current })
        : await listByFlag(params.tagId, { pageNum: this.current });
      console.log(data);
      if (data.list.length) {
        this.current++;
